...
namespace ECommerce.Customer.Api.Services
{
    class CreateCustomerIntegrationHandler : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            await _bus.Rpc.RespondAsync<CreateUserCommand, ValidationResult>(async request => await AdicionarCliente(request), stoppingToken);
            
            await Task.CompletedTask;
        }

        private async Task<ValidationResult> AdicionarCliente(CreateUserCommand request)
        {
