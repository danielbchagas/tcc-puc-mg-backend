...
using EasyNetQ;
using ECommerce.Customers.Api.Models;
...
{
    public class CreatePhoneIntegrationHandler : BackgroundService
    {
        private readonly RabbitMqOption _rabbitMQOptions;
        private readonly IServiceProvider _serviceProvider;
        private IBus _bus;

        public CreatePhoneIntegrationHandler(IOptions<RabbitMqOption> rabbitMQOptions, IServiceProvider serviceProvider)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            await _bus.Rpc.RespondAsync<CreatePhoneCommand, ValidationResult>(async request => await AdicionarTelefone(request), stoppingToken);

            await Task.CompletedTask;
        }

        private async Task<ValidationResult> AdicionarTelefone(CreatePhoneCommand request)
        {
